...
extern bool bPatternPaintEnable;
extern int patternPaint;
extern char (*ptr_multicolour_dither_3stepSateen[50])[8][8];
...
const int originalFormHgt		=400;
const int originalFormWdt		=296;
...
void __fastcall TFormCHREditor::Draw(bool drawAll)
{
	...
	bool drp=FormMain->Displayrepeatingpatternsmode1->Checked;
	...
	if(N1x21->Checked) {fx=0; fy=1;}
	if(N2x11->Checked) {fx=1; fy=0;}

	if(N2x21->Checked) {fx=1; fy=1;}
	if(N3x31->Checked) {fx=2; fy=2;}
	if(N4x41->Checked) {fx=3; fy=3;}
	...
	*/
	...
	//pre-row
	if(a){
						DrawCHR(-64        			,-64        	,drp? tileActive+((fy)*16)+fx:(ty>0&&tx>0  ?tileActive-17:-1)); //NW
						DrawCHR( 64+8      			,-64        	,drp? tileActive+((fy)*16)   :(ty>0        ?tileActive-16:-1)); //N
						DrawCHR( 64+8+128+nx			,-64    	,drp? tileActive+((fy)*16)+(fx?1:0)   :(ty>0&&tx<15 ?tileActive-15:-1));

	if(b && fx==1) 		DrawCHR( 64+8+128+nx+128+8	,-64        ,drp? tileActive+((fy)*16)  :(ty>0&&tx<14 ?tileActive-14:-1));

	else if(b && fx==2){ DrawCHR( 64+8+128+nx+128+0	,-64        ,drp? tileActive+((fy)*16)+2   :(ty>0&&tx<14 ?tileActive-14:-1));
						DrawCHR( 64+8+256+nx+128+8	,-64        ,drp? tileActive+((fy)*16)   :(ty>0&&tx<13 ?tileActive-13:-1));
					  }
	else if(b && fx==3){ DrawCHR( 64+8+128+nx+128+0	,-64        ,drp? tileActive+((fy)*16)+2   :(ty>0&&tx<14 ?tileActive-14:-1));
						DrawCHR( 64+8+256+nx+128+0	,-64        ,drp? tileActive+((fy)*16)+3   :(ty>0&&tx<13 ?tileActive-13:-1));
						DrawCHR( 64+8+384+nx+128+8	,-64        ,drp? tileActive+((fy)*16)     :(ty>0&&tx<12 ?tileActive-12:-1));
					  }

	}

	//1st row from tileactive
	if(a)						DrawCHR(-64        				, 64+8      ,drp? tileActive+fx			:(tx>0        ?tileActive-1 :-1));
								DrawCHR( 64+8      				, 64+8      ,tileActive);

	if(b || a)					DrawCHR( 64+8+128+nx			, 64+8 		,drp? tileActive+(fx?1:0)	:(tx<15       ?tileActive+1 :-1));
	if(b && (fx==2 || fx==3))   DrawCHR( 64+8+256+nx    		, 64+8 		,drp? tileActive+2 :(tx<14       ?tileActive+2 :-1));
	if(b && (fx==3)) 			DrawCHR( 64+8+384+nx			, 64+8 		,drp? tileActive+3 :(tx<13       ?tileActive+3 :-1));

	if(b && a)  				DrawCHR( 64+8+128*fx+0+128+8	, 64+8      ,drp? tileActive : (tx<15-fx       ?tileActive+1+fx :-1));


	//2nd
	if(b || a)					DrawCHR(-64        				, 64+8+128+ny , drp? tileActive+fx+(fy?16:0) :(ty<15&&tx>0 ?tileActive+15:-1));

	if(b || a)					DrawCHR( 64+8      				, 64+8+128+ny , drp? tileActive+(fy?16:0):(ty<15       ?tileActive+16:-1));
	if(b || a)					DrawCHR( 64+8+128+nx			, 64+8+128+ny , drp? tileActive+(fx?1:0)+(fy?16:0): (ty<15&&tx<15?tileActive+17:-1));

	if(b && (fx==2 || fx==3))   DrawCHR( 64+8+256+nx   			, 64+8+128+ny , drp? tileActive+2+(fy?16:0) :(tx<14       ?tileActive+18 :-1));
	if(b && (fx==3))  			DrawCHR( 64+8+384+nx   			, 64+8+128+ny , drp? tileActive+3+(fy?16:0) :(tx<13       ?tileActive+19 :-1));

	if(b)   					DrawCHR( 64+8+128*fx+0+128+8	, 64+8+128+ny , drp? tileActive+(fy?16:0) :(ty<15&&tx<15-fx?tileActive+17+fx:-1));

	//3rd
	if(fy==2 || fy==3){

		if(b )						DrawCHR(-64        			, 64+8+256+ny		,drp? tileActive+fx+(fy?32:0) :	(ty<15&&tx>0 ?tileActive+31:-1));
		if(b )						DrawCHR( 64+8      			, 64+8+256+ny		,drp? tileActive+(fy?32:0) :(ty<15       ?tileActive+32:-1));
		if(b )						DrawCHR( 64+8+128+nx			, 64+8+256+ny	,drp? tileActive+(fx?1:0)+(fy?32:0):(ty<15&&tx<15?tileActive+33:-1));

		if(b && (fx==2 || fx==3))	DrawCHR( 64+8+256+nx   , 64+8+256 ,drp? tileActive+2+(fy?32:0) :(tx<14       ?tileActive+34 :-1));
		if(b && (fx==3))  		 	DrawCHR( 64+8+384+nx   , 64+8+256 ,drp? tileActive+3+(fy?32:0) :(tx<13       ?tileActive+35 :-1));

	if(b)   						DrawCHR( 64+8+128*fx+0+128+8	, 64+8+256+ny,drp? tileActive+(fy?32:0) :(ty<15&&tx<15-fx?tileActive+33+fx:-1));
	...
	if(fy==3){
		...
		if(b )						DrawCHR(-64        					, 64+8+384+ny,drp? tileActive+fx+(fy?48:0) :	(ty<15&&tx>0 ?tileActive+47:-1));
		if(b )						DrawCHR( 64+8      					, 64+8+384+ny,drp? tileActive+(fy?48:0) :(ty<15       ?tileActive+48:-1));
		if(b )						DrawCHR( 64+8+128+nx				, 64+8+384+ny,drp? tileActive+(fx?1:0)+(fy?48:0):(ty<15&&tx<15?tileActive+49:-1));

		if(b && (fx==2 || fx==3))  	DrawCHR( 64+8+256+nx   , 64+8+384 ,drp? tileActive+2+(fy?48:0) :(tx<14       ?tileActive+50 :-1));   //f==2 is redundant here.
		if(b && (fx==3))  		 	DrawCHR( 64+8+384+nx   , 64+8+384 ,drp? tileActive+3+(fy?48:0) :(tx<13       ?tileActive+51 :-1));

	if(b)   						DrawCHR( 64+8+128*fx+0+128+8	, 64+8+384+ny,drp? tileActive+(fy?48:0) :(ty<15&&tx<15-fx?tileActive+49+fx:-1));
	...
	if(b && a)
	{
								DrawCHR(-64        			, 64+8+128*fy+0+128+8		, drp? tileActive+fx:(ty<14&&tx>0 ?tileActive+15+fy*16:-1));
								DrawCHR( 64+8      			, 64+8+128*fy+0+128+8		, drp? tileActive	:(ty<14       ?tileActive+16+fy*16:-1));
								DrawCHR( 64+8+128+nx		, 64+8+128*fy+0+128+8		, drp? tileActive+(fx?1:0): (ty<14&&tx<15?tileActive+17+fy*16:-1));

			if(fx==1) 			DrawCHR( 64+8+128+nx+128+8	, 64+8+128*fy+0+128+8		,drp? tileActive  :(ty<14&&tx<14? tileActive+18+fy*16:-1));

			else if(fx==2){ 	DrawCHR( 64+8+128+nx+128+0	, 64+8+128*fy+0+128+8       ,drp? tileActive+2:(ty<14&&tx<14 ?tileActive+18+fy*16:-1));
								DrawCHR( 64+8+256+nx+128+8	, 64+8+128*fy+0+128+8       ,drp? tileActive  :(ty<13&&tx<13 ?tileActive+19+fy*16:-1));
					  }
			else if(fx==3){ 	DrawCHR( 64+8+128+nx+128+0	, 64+8+128*fy+0+128+8       ,drp? tileActive+2  :(ty<14&&tx<14 ?tileActive+18+fy*16:-1));
								DrawCHR( 64+8+256+nx+128+0	, 64+8+128*fy+0+128+8       ,drp? tileActive+3  :(ty<13&&tx<13 ?tileActive+19+fy*16:-1));
								DrawCHR( 64+8+384+nx+128+8	, 64+8+128*fy+0+128+8       ,drp? tileActive    :(ty<12&&tx<12 ?tileActive+20+fy*16:-1));
					  }
	...
	if(mn){
		...
		for(int cy=0;cy<dy;cy++){
			for(int cx=0;cx<dx;cx++){
				...
				buffer->Canvas->MoveTo(cx*128+127+8, cy*128+127+7);
				buffer->Canvas->LineTo(cx*128+127+8, cy*128+127+10);
				buffer->Canvas->MoveTo(cx*128+127+7, cy*128+127+8);
				buffer->Canvas->LineTo(cx*128+127+10,cy*128+127+8);
			}
		}
	}
	if(b){
		if(tg){
			...
			buffer->Canvas->LineTo(64+128*fx+127+8, 	64+127+8);

			buffer->Canvas->MoveTo(64+127+8, 		64+8);
			buffer->Canvas->LineTo(64+127+8, 		64+128*fy+127+8);

			if(fy==2 || fy==3 || fx==2 || fx==3){
				buffer->Canvas->MoveTo(64+8, 				64+127+128+8);
				buffer->Canvas->LineTo(64+128*fx+127+8, 	64+127+128+8);

				buffer->Canvas->MoveTo(64+127+128+8, 		64+8);
				buffer->Canvas->LineTo(64+127+128+8, 		64+128*fy+127+8);
			}
			if(fy==3 || fx==3){
				buffer->Canvas->MoveTo(64+8, 				64+127+256+8);
				buffer->Canvas->LineTo(64+128*fx+127+8, 	64+127+256+8);

				buffer->Canvas->MoveTo(64+127+256+8, 	64+8);
				buffer->Canvas->LineTo(64+127+256+8, 	64+128*fy+127+8);
			}
		...
		if(mn){
			...
			buffer->Canvas->MoveTo(64+128*fx+127+8, 64+127+8);
			buffer->Canvas->LineTo(64+128*fx+127+4, 64+127+8);

			buffer->Canvas->MoveTo(64+127+8, 64+128*fy+127+8);
			buffer->Canvas->LineTo(64+127+8, 64+128*fy+127+4);
			...
			if(fy==2 || fy==3 || fx==2 || fx==3){
				...
				buffer->Canvas->MoveTo(64+128*fx+127+8, 64+127+128+8);
				buffer->Canvas->LineTo(64+128*fx+127+4, 64+127+128+8);

				buffer->Canvas->MoveTo(64+127+128+8, 64+128*fy+127+8);
				buffer->Canvas->LineTo(64+127+128+8, 64+128*fy+127+4);
			}
			...
			if(fy==3 || fx==2){
				...
				buffer->Canvas->MoveTo(64+128*fx+127+8, 64+127+256+8);
				buffer->Canvas->LineTo(64+128*fx+127+4, 64+127+256+8);

				buffer->Canvas->MoveTo(64+127+256+8, 64+128*fy+127+8);
				buffer->Canvas->LineTo(64+127+256+8, 64+128*fy+127+4);
			}
		}
	}
...
void __fastcall TFormCHREditor::MirrorHorizontal(bool u)
{
	int i,j,pp,tmp;
	int x,y;
	...
	if(b){
		...
		for (int h = 0; h < tileSelRectHgt; h++) {
			for (int w = 0; w < tileSelRectWdt / 2; w++) {
				int leftIdx  = ta + h * 16 + w;
				int rightIdx = ta + h * 16 + (tileSelRectWdt - 1 - w);

				if (ButtonBitmaskLo->Down || !FormMain->Applytorotate1->Checked) {

					memcpy(tempchr, &chr[tileViewTable[leftIdx] * 16 + bankViewTable[set + leftIdx]], 8);
					memcpy(&chr[tileViewTable[leftIdx] * 16 + bankViewTable[set + leftIdx]],
						&chr[tileViewTable[rightIdx] * 16 + bankViewTable[set + rightIdx]], 8);
					memcpy(&chr[tileViewTable[rightIdx] * 16 + bankViewTable[set + rightIdx]], tempchr, 8);
				}

				if (ButtonBitmaskHi->Down || !FormMain->Applytorotate1->Checked) {

					memcpy(tempchr, &chr[tileViewTable[leftIdx] * 16 + bankViewTable[set + leftIdx] + 8], 8);
					memcpy(&chr[tileViewTable[leftIdx] * 16 + bankViewTable[set + leftIdx] + 8],
						&chr[tileViewTable[rightIdx] * 16 + bankViewTable[set + rightIdx] + 8], 8);
					memcpy(&chr[tileViewTable[rightIdx] * 16 + bankViewTable[set + rightIdx] + 8], tempchr, 8);
				}
			}
		}
	}
...
void __fastcall TFormCHREditor::MirrorVertical(bool u)
{
	int i,pp;
	int x,y;
    int ba=bankActive;
	...
	if(b){
		unsigned char tempchr[8];
		for (int w = 0; w < tileSelRectWdt; w++) {
			for (int h = 0; h < tileSelRectHgt / 2; h++) {
				int topIdx  = ta + w  + h * 16;
				int bottomIdx = ta + w  + (tileSelRectHgt - 1 - h) * 16;

				if (ButtonBitmaskLo->Down || !FormMain->Applytorotate1->Checked) {

					memcpy(tempchr, &chr[tileViewTable[topIdx] * 16 + bankViewTable[set + topIdx]], 8);
					memcpy(&chr[tileViewTable[topIdx] * 16 + bankViewTable[set + topIdx]],
						&chr[tileViewTable[bottomIdx] * 16 + bankViewTable[set + bottomIdx]], 8);
					memcpy(&chr[tileViewTable[bottomIdx] * 16 + bankViewTable[set + bottomIdx]], tempchr, 8);
				}

				if (ButtonBitmaskHi->Down || !FormMain->Applytorotate1->Checked) {

					memcpy(tempchr, &chr[tileViewTable[topIdx] * 16 + bankViewTable[set + topIdx] + 8], 8);
					memcpy(&chr[tileViewTable[topIdx] * 16 + bankViewTable[set + topIdx] + 8],
						&chr[tileViewTable[bottomIdx] * 16 + bankViewTable[set + bottomIdx] + 8], 8);
					memcpy(&chr[tileViewTable[bottomIdx] * 16 + bankViewTable[set + bottomIdx] + 8], tempchr, 8);
				}
			}
		}
	}
...
void __fastcall TFormCHREditor::Fill(TShiftState Shift,int x,int y, int tx, int ty, int extpp, int bufpp, bool isNametable)                  //int tempPal, int fillPal,
{
	...
	bool bStripSet = FormMain->btnStripSet->Down;
	...
	if (penActive==0&&(!Shift.Contains(ssShift)))
	{
		for(i=offy;i<leny;i++)
		{
			for(j=offx;j<lenx;j++)
			{
				if(bPatternPaintEnable && FormPatternPaint->btnBucket->Down)
				{
					int	iPatternPaint = (*ptr_multicolour_dither_3stepSateen[patternPaint +(bStripSet? 25:0)])[(i-offy)%8][(j-offx)%8];

					if(!FormPatternPaint->btnMaskMode->Down){
						replaceColField= bFieldPen?  iPatternPaint:replaceColField;
						replaceColGap=   bGapPen?    iPatternPaint:replaceColGap;
					}
					if(
					   (iPatternPaint == 0 && FormPatternPaint->btn0->Down)
					   || (iPatternPaint == 1 && FormPatternPaint->btn1->Down)
					   || (iPatternPaint == 2 && FormPatternPaint->btn2->Down)
					   || (iPatternPaint == 3 && FormPatternPaint->btn3->Down)
					){
...
void __fastcall TFormCHREditor::Line(TShiftState Shift,int mdx, int mdy, int x,int y,int mode)
{
  ...
  bool bStripSet = FormMain->btnStripSet->Down;
  bool bQuick = FormLineDetails->btnQuick->Down || bAltQuick;
	...
	for(ty=0;ty<h;ty++){
		for(int py=0;py<8;py++){
			for(int px=0;px<8;px++){
				...
				{
					if(!bSmudge){
                            if(bPatternPaintEnable &&
									(
										  (!btnThick->Down 		&& FormPatternPaint->btnPen->Down)
										||(btnThick->Down 		&& FormPatternPaint->btnBrush->Down)
									)
								){
									int iPatternPaint = (*ptr_multicolour_dither_3stepSateen[patternPaint + (bStripSet? 25:0)])[py][px];
									...
									if(
										...
										|| (iPatternPaint == 3 && FormPatternPaint->btn3->Down)
                                            ...
                                            if(FormPatternPaint->btnMaskMode->Down) iPatternPaint=bgPalCur;
											if(ButtonBitmaskLo->Down||!FormMain->Applytopen1->Checked) chr[pp]=(chr[pp]&~mask)|(((iPatternPaint&1)<<7)>>px);
											if(ButtonBitmaskHi->Down||!FormMain->Applytopen1->Checked) chr[pp+8]=(chr[pp+8]&~mask)|(((iPatternPaint&2)<<6)>>px);
										}
								}
								else {
									if(ButtonBitmaskLo->Down||!FormMain->Applytopen1->Checked) chr[pp]=(chr[pp]&~mask)|(((bgPalCur&1)<<7)>>px);
...
void __fastcall TFormCHREditor::TileChange(int xoff,int yoff)
{
	int tx,ty;
	bool b=btn2x2mode->Down;
	int nx= 16-chrSelection.Width();
	int ny= 16-chrSelection.Height();
	tx=(tileActive&15)+xoff;
	ty=(tileActive/16)+yoff;

   //	if(tx<0||tx>nx||ty<0||ty>ny) return;
	int xcomp=0; int ycomp=0;
	if(tx<0) xcomp=tx-nx;
	if(ty<0) ycomp=ty-ny;

	//if(tx>nx) xcomp= (tx-nx);
	//if(ty>ny) ycomp= (ty-ny);

	if(tx>nx) xcomp= nx + xoff;
	if(ty>ny) ycomp= ny + yoff;

	FormMain->SetTile(tileActive+=((yoff-ycomp)*16+(xoff-xcomp)));
	if(b) {}
...
TMouseButton Button, TShiftState Shift, int X, int Y)
{
	...
	bAltQuick = (bBufAlt && !bBufShift && !bBufCtrl && Shift.Contains(ssLeft));
	bool bQuick = FormLineDetails->btnQuick->Down || (bAltQuick);
	...
	*/
	...
	int skipx= ((Shift.Contains(ssRight)||Shift.Contains(ssShift)||Shift.Contains(ssCtrl)))? fx+1:1;
	int skipy= ((Shift.Contains(ssRight)||Shift.Contains(ssShift)||Shift.Contains(ssCtrl)))? fy+1:1;

	//if(!Shift.Contains(ssRight)){
		if(Y<64+cm)                                                                  
		{
			if(X<64+cm) TileChange(-skipx,-skipy);
			if(X>=64+8-pm&&X<64+8+128+nx+pm) TileChange(0,-skipy);
			if(X>=64+8+128+8+nx-cm) TileChange(skipx,-skipy);
			bForbidPaint=true;
		}

		if(Y>=64+8-pm&&Y<64+8+128+ny+pm)
		{
			if(X<64+cm) TileChange(-skipx,0);
			if(X>=64+8+128+8+nx-cm) TileChange(skipx,0);
			bForbidPaint=true;
		}

		if(Y>=64+8+128+8+ny-cm)
		{
			if(X<64+cm) TileChange(-skipx,skipy);
			if(X>=64+8-pm&&X<64+8+128+nx+pm) TileChange(0,skipy);
			if(X>=64+8+128+8+nx-cm) TileChange(skipx,skipy);
	...
	if(Y>=lo-pm && Y<hiy+pm && X>=lo-pm && X<hix+pm)
	{


		lineDrawing = ((btnLine->Down)&& (!Shift.Contains(ssCtrl)) ) || (bAltQuick);
		...
		if(bQuick)
		{
			...
			lineDownX= (lineUpX < hix)? lineUpX:hix-1;
			lineDownY= (lineUpY > lo)? lineUpY:lo;
			lineDownY= (lineUpY < hiy)? lineUpY:hiy-1;
			...
			lineDownY=Y;
			lineDownX= (X > lo)? X:lo;
			lineDownX= (X < hix)? X:hix-1;
			lineDownY= (Y > lo)? Y:lo;
			lineDownY= (Y < hiy)? Y:hiy-1;
		}
...
TShiftState Shift, int X, int Y)
{
	...
	*/
	...
	int gx,gy,gfx,gfy; //guards for thick pen
	...
	if((Y>=lo-pm && Y<hiy+pm && X>=lo-pm && X<hix+pm) || (lineDrawing))
	{
		...
		if(Shift.Contains(ssLeft))
		{
			 ...
			 for(int br_x=0; br_x<iThick; br_x++)
			 {
				if(btnThick->Down)
				{
					...
					//gfx stands for guard, makes sure we don't draw outside.
					gx=((X-(64+8)+ix)/16);
					gy=((Y-(64+8)+iy)/16);
						...
						if (penActive==0&&(!Shift.Contains(ssShift)))
						{
							if(!bSmudge){
								...
								if(bPatternPaintEnable &&
									...
									iPatternPaint = (*ptr_multicolour_dither_3stepSateen[patternPaint + (bStripSet? 25:0)])[py][px];
...
TShiftState Shift)
{
	...
	if(!Shift.Contains(ssCtrl))
	{
		...
		if(Key==VK_OEM_3 || Key==VK_OEM_5) {
						bPatternPaintEnable^=true;
							if(bPatternPaintEnable) FormMain->LabelStats->Caption="Pattern paint ON.";
							else                    FormMain->LabelStats->Caption="Pattern paint OFF (normal colour paint on).";
							FormMain->StatusUpdateWaiter->Enabled=true;
							holdStats=true;
							FormMain->DrawPalettes();
						}
...
{
	if(!lineDrawing)MirrorHorizontal(true);
}
//---------------------------------------------------------------------------

void __fastcall TFormCHREditor::SpeedButtonVFlipClick(TObject *Sender)
{
	if(!lineDrawing)MirrorVertical(true);
}
//---------------------------------------------------------------------------
...
void __fastcall TFormCHREditor::FormCreate(TObject *Sender)
{
	...
	if(prefStartPinCHR)  {
		FormCHREditor->PopupMode=pmExplicit;
		FormMain->PinCHREditor1->Checked=true;
	} else{
		FormCHREditor->PopupMode=pmNone;
		FormMain->PinCHREditor1->Checked=false;
	}
	FormCHREditor->StayOnTop1->Checked= FormMain->PinNavigator1->Checked? pmExplicit : pmNone;
}
//---------------------------------------------------------------------------
	  ...
	  TShiftState Shift, int WheelDelta, TPoint &MousePos, bool &Handled)
{
	...
	else
	{
		   ...
		   if(WheelDelta<0)
			{
				...
				if(Shift.Contains(ssShift)||Shift.Contains(ssCtrl))
				{
					...
					if(Shift.Contains(ssAlt)){bgPalCur=(bgPalCur-1)&3; FormMain->DrawPalettes(); }
					else {
						patternPaint++;

						if(patternPaint>=25) patternPaint=0;
			...
			else
			{
				if(Shift.Contains(ssShift)||Shift.Contains(ssCtrl))
				{
					...
					if(Shift.Contains(ssAlt)){bgPalCur=(bgPalCur+1)&3; FormMain->DrawPalettes();}
						...
						if(patternPaint<0) patternPaint=24;
						bUpdPalette=true;
					}
				}
			}
		...
		if(bUpdPalette) FormMain->DrawPalettes();
		}
	  ...
	  TMouseButton Button, TShiftState Shift, int X, int Y)
{
	...
	//bool bMove  = FormLineDetails->btnMove->Down;
	//bool bReuse = FormLineDetails->btnReuse->Down;
	bAltQuick = (bBufAlt && !bBufShift && !bBufCtrl);
	bool bQuick = FormLineDetails->btnQuick->Down || bAltQuick;
	bIsFilling=false;
	lineUpX=X;
		lineUpY=Y;
...
void __fastcall TFormCHREditor::N2x21Click(TObject *Sender)
{
	...
	switch (tag) {
		case 1:  btn2x2mode->Caption="&Edit 3x3 tiles"; FormMain->N2x2tileeditmode1->Caption="3&x3 CHR Editor mode"; fx=2; fy=2; break;
		case 2:  btn2x2mode->Caption="&Edit 4x4 tiles"; FormMain->N2x2tileeditmode1->Caption="4&x4 CHR Editor mode"; fx=3; fy=3; break;

		case 12: btn2x2mode->Caption="&Edit 1x2 tiles"; FormMain->N2x2tileeditmode1->Caption="1&x2 CHR Editor mode"; fx=0; fy=1; break;
		case 21: btn2x2mode->Caption="&Edit 2x1 tiles"; FormMain->N2x2tileeditmode1->Caption="2&x1 CHR Editor mode"; fx=1; fy=0; break;

	default:
		// case 0
		btn2x2mode->Caption="&Edit 2x2 tiles"; FormMain->N2x2tileeditmode1->Caption="2&x2 CHR Editor mode"; fx=1;  fy=1; break;
	...
	if(b){
		...
		FormCHREditor->Width =originalFormWdt+nx;
		FormCHREditor->Height=originalFormHgt+ny;

		GroupBox1->Width	=originalGroupBoxWdt+nx;
		GroupBox1->Height	=originalGroupBoxHgt+ny;

		PaintBoxChr->Width  =originalPaintBox+nx;
		PaintBoxChr->Height  =originalPaintBox+ny;
...
void __fastcall TFormCHREditor::FormResize(TObject *Sender)
{
...
void __fastcall TFormCHREditor::TimerResizeTimer(TObject *Sender)
{

	CHReditor_tw=    (FormCHREditor->Width -(296-56))/128;
	CHReditor_th=    (FormCHREditor->Height -(400-56))/128;
	if(CHReditor_tw<0) CHReditor_tw=0;
	if(CHReditor_tw>3) CHReditor_tw=3;
	if(CHReditor_th<0) CHReditor_th=0;
	if(CHReditor_th>3) CHReditor_th=3;

	if(CHReditor_tw>0 || CHReditor_th>0)  btn2x2mode->Down=true;
	else btn2x2mode->Down=false;
	bool b=btn2x2mode->Down;
	int nx=b?128 * CHReditor_tw:0;
	int ny=b?128 * CHReditor_th:0;
	...
	if(b)
		...
		btn2x2mode->Caption="&Edit "+IntToStr(CHReditor_tw+1)+"x"+IntToStr(CHReditor_th+1)+" tiles";
		FormMain->N2x2tileeditmode1->Caption=IntToStr(CHReditor_tw+1)+"&x"+IntToStr(CHReditor_th+1)+" CHR Editor mode";
		...
		if(FormCHREditor->Visible) Draw(true);
	TimerResize->Enabled=false;
	TimerResize2->Enabled=false;
	TimerResize2->Enabled=true;

}
//---------------------------------------------------------------------------

void __fastcall TFormCHREditor::TimerResize2Timer(TObject *Sender)
{
	bool b=btn2x2mode->Down;
   int nx=b?128 * CHReditor_tw:0;
   int ny=b?128 * CHReditor_th:0;
...
void __fastcall TFormCHREditor::StayOnTop1Click(TObject *Sender)
{
	if(StayOnTop1->Checked) FormCHREditor->PopupMode=pmExplicit;
	else FormCHREditor->PopupMode=pmNone;
