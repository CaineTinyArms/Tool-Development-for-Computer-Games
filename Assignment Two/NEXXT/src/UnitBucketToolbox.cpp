...
void __fastcall TFormBucketToolbox::FormCreate(TObject *Sender)
{
	...
	if(prefStartPinBucket)  {
		FormBucketToolbox->PopupMode=pmExplicit;
		FormMain->PinBuckettoolbox1->Checked=true;
	} else{
		FormBucketToolbox->PopupMode=pmNone;
		FormMain->PinBuckettoolbox1->Checked=false;
	}
}
//---------------------------------------------------------------------------
