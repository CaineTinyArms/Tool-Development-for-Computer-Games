...
void __fastcall TFormMetatileTool::FormCreate(TObject *Sender)
{
	...
	if(prefStartShowMetatileEditor) FormMetatileTool->Show();
	if(prefStartPinMetatileEditor)  {
		FormMetatileTool->PopupMode=pmExplicit;
		FormMain->PinMetatiletool1->Checked=true;
	} else{
		FormMetatileTool->PopupMode=pmNone;
		FormMain->PinMetatiletool1->Checked=false;
	}
	...
	for (int i=0; i < nameTableHeight ; i+=d) {
		...
		for (int j = 0; j < nameTableWidth ; j+=d) {
			...
			if(iMapMatchCnt==tmp_cnt) {
				...
				nameTableViewX=nameSelection.left-16;
				nameTableViewY=nameSelection.top-16;
				FormMain->CorrectView();
