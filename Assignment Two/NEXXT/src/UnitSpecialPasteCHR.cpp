...
void __fastcall TFormSpecialPasteCHR::FormCreate(TObject *Sender)
{
	if(prefStartShowPaste) FormSpecialPasteCHR->Visible=true;
	if(prefStartPinPaste)  {
		FormSpecialPasteCHR->PopupMode=pmExplicit;
		FormMain->PinSpecialpastetoolbox1->Checked=true;
	} else{
		FormSpecialPasteCHR->PopupMode=pmNone;
		FormMain->PinSpecialpastetoolbox1->Checked=false;
	}
}
//---------------------------------------------------------------------------
