...
void __fastcall TFormSwapBanks::FormCreate(TObject *Sender)
{
	...
	if(prefStartShowBankSwapper) FormSwapBanks->Show();
	if(prefStartPinBankSwapper)  {
		FormSwapBanks->PopupMode=pmExplicit;
		FormMain->PinCHRbankswapper1->Checked=true;
	} else{
		FormSwapBanks->PopupMode=pmNone;
		FormMain->PinCHRbankswapper1->Checked=false;
	}
}
//---------------------------------------------------------------------------
