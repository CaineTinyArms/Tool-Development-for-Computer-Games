...
extern bool mmc2switch;
extern bool mmc2_Doublepair_1stSet;

extern bool mmc2_checked1;
...
extern bool bNavDoSnap;
extern bool	bNavDoAlign;
extern bool	bNavDoZoomchange;
...
extern int nameTableViewX2;
extern int nameTableViewY2;
extern int nxDown;
extern int nyDown;
extern int nameXC;
extern int nameYC;
...
//---------------------------------------------------------------------------
void __fastcall TFormNavigator::Draw(bool doSnap, bool doAlign, bool halfToFull)
{
	...
	if(halfToFull){
		FormNavigator->ClientWidth  = FormNavigator->ClientWidth*2;
		FormNavigator->ClientHeight = FormNavigator->ClientHeight*2 ;
		if(abs(FormNavigator->ClientWidth - minW*8*fStretchScale)<windowSizeSnapRange*2)  FormNavigator->ClientWidth=minW*8*fStretchScale;
		if(abs(FormNavigator->ClientHeight - minH*8*fStretchScale)<windowSizeSnapRange*2) FormNavigator->ClientHeight=minH*8*fStretchScale;
	...
	if(doSnap){
		...
		if(abs(FormNavigator->ClientWidth - minW*8*fStretchScale)<windowSizeSnapRange*1)  FormNavigator->ClientWidth=minW*8*fStretchScale;
		if(abs(FormNavigator->ClientHeight - minH*8*fStretchScale)<windowSizeSnapRange*1) FormNavigator->ClientHeight=minH*8*fStretchScale;

		//17*2 too small -arbitrarily chosen based on what feels good.
		//if(FormNavigator->ClientWidth - minW*8*fStretchScale<windowSizeSnapRange*1.4)  FormNavigator->ClientWidth=minW*8*fStretchScale;
		//if(FormNavigator->ClientHeight - minH*8*fStretchScale<windowSizeSnapRange*1.4) FormNavigator->ClientHeight=minH*8*fStretchScale;
	...
	//int lrAlign = FormNavigator->ClientWidth>Map1->Width? (FormNavigator->ClientWidth - Map1->Width)/2:0;
	//int tbAlign = FormNavigator->ClientHeight>Map1->Height?(FormNavigator->ClientHeight - Map1->Height)/2:0;
int barVal=windowSizeSnapRange;

	if(doAlign)
	{
        //disabled until i figure out why it's giving false positives
	...
	//todo: figure out how to use threaded objects properly
				...
				for(i=0;i<minH;i++)
				{
					...
					for(j=0;j<minW;j++)
					{
						if(j>=0&&j<w&&i>=0&&i<h)
						{
							...
							highlight_tile=nameTable[((i*w)/(w*cgY))*(w*cgY) + (j/cgX)*cgX];
							/*
							...
							{ */
								...
								{
									FormMain->DrawTile(Map1->Picture,x,y,tile,highlight_tile,FormMain->AttrGet(j,i,false,false),j,i,false,false,1,true,false,bHalfsize,false,false);
								}
								else
								{
		 ...
		 if(bMouseOverNav){
			if(nameTableViewX2!=nameTableViewX || nameTableViewY2!=nameTableViewY){
				r.left  	=(nameTableViewX2);
				r.right 	=(nameTableViewX2+viewPortWidth);
				r.top   	=(nameTableViewY2);
				r.bottom	=(nameTableViewY2+30);

				if(nameTableWidth<viewPortWidth)
				{
					r.left=r.left +((viewPortWidth-w)/2);
					r.right=r.right +((viewPortWidth-w)/2);
				}
				...
				Map1->Canvas->Pen->Color=TColor(0x990099);
				...
				if (bHalfsize) Map1->Canvas->Pen->Color=TColor(0x990099);
				else Map1->Canvas->Pen->Color=TColor(0x220022);
			...
			r.right 	=(nameTableViewX+viewPortWidth);
		...
		for(i=0;i<th;i++)
		{
			...
			for(j=0;j<tw;j++)
			{
				...
				if(j>=0 && tx+j<w && i>=0 && ty+i<h) 
				{
					...
					highlight_tile=nameTable[(((ty+i)*w)/(w*cgY))*(w*cgY) + ((tx+j)/cgX)*cgX];
					/*
					if(FormMain->SpeedButtonChecker->Down)
					{
					...
					{ */
						if(!FormMain->SpeedButtonSelTiles->Down||chrSelected[tile])
						{
							FormMain->DrawTile(Map1->Picture,x,y,tile,highlight_tile,FormMain->AttrGet(tx+j,ty+i,false,false),tx+j,ty+i,false,false,1,true,false,bHalfsize,false,false);
						}
						else
						{
...
void __fastcall TFormNavigator::FormShow(TObject *Sender)
{
	...
	DelayedDraw->Enabled=false;  //reset timer on every resize event
	DelayedDraw->Interval=100;
	DelayedDraw->Enabled=true;
...
void __fastcall TFormNavigator::FormResize(TObject *Sender)
{
	...
	//::Draw(bool doSnap, bool doAlign, bool halfToFull)
...
void __fastcall TFormNavigator::ResizeTimerTimer(TObject *Sender)
{
  ...
  //Draw(true,true,false); //second pass.
  ResizeTimer->Enabled=false;
	...
	/*
	int lrAlign = FormNavigator->ClientWidth > Map1->Width? abs(FormNavigator->ClientWidth - Map1->Width)/2:0;
	int tbAlign = FormNavigator->ClientHeight > Map1->Height? abs(FormNavigator->ClientHeight - Map1->Height)/2:0;
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
	...
	if (Shift.Contains(ssRight)) { //prep context for inside/outside selection
		FormMain->SelectTile(nameTable[ny*nameTableWidth+nx]);
	...
	else if (Shift.Contains(ssLeft))  //drag view
	{
				...
				|(nameTableViewY>ny))
		{
				...
				nameTableViewX=nx-16;
				nameTableViewY=ny-15;
...
void __fastcall TFormNavigator::FormCreate(TObject *Sender)
{
	...
	if(prefStartShowNavigator) FormNavigator->Show();
	if(prefStartPinNavigator)  {
		FormNavigator->PopupMode=pmExplicit;
		FormMain->PinNavigator1->Checked=true;
	} else{
		FormNavigator->PopupMode=pmNone;
		FormMain->PinNavigator1->Checked=false;
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TFormNavigator::FormMouseWheel(TObject *Sender,
      TShiftState Shift, int WheelDelta, TPoint &MousePos, bool &Handled)
{
	 //if there were a scrollbox in the future.

	 if (Shift.Contains(ssShift) && HorzScrollBar->Visible) {
		if (WheelDelta > 0)
			HorzScrollBar->Position -= 16;//HorzScrollBar->Increment;
		else
			HorzScrollBar->Position += 16;//HorzScrollBar->Increment;
	} else if (VertScrollBar->Visible){

		if (WheelDelta > 0)
			VertScrollBar->Position -= 16; //VertScrollBar->Increment;
		else
			VertScrollBar->Position += 16; //VertScrollBar->Increment;
	}
...
void __fastcall TFormNavigator::DelayedDrawTimer(TObject *Sender)
{
	if(!openByFileDone) return;

	DelayedDraw->Enabled=false;
	CueDrawTimer->Enabled=false;
	Draw( bNavDoSnap, bNavDoAlign, bNavDoZoomchange);
