...
void __fastcall TFormCHRbit::TilepropertiestoclipboardasCheader1Click(
      TObject *Sender)
{

	char str[1024*16],buf[1024];
	char separator[3];
	int i,j;
	AnsiString name;
	AnsiString bank;
	name=RemoveExt(ExtractFileName(FormMain->SaveDialogSession->FileName));
	...
	int set=bankActive/15;
	//strcpy
	/*
	if((bExportPalFilename)&&(bExportPalSet))
		sprintf(str,"const unsigned char palette_%s_%s[256]={ ",name.c_str(),bank.c_str());
	else if (bExportPalFilename)
		sprintf(str,"const unsigned char palette_%s[256]={ ",name.c_str());
	else if (bExportPalSet)
		sprintf(str,"const unsigned char palette_%s[256]={ ",bank.c_str());
	else
		sprintf(str,"const unsigned char palette[256]={ ");
	*/
	sprintf(str,"const unsigned char tileprop_%s_%s[256]={\n",name.c_str(),bank.c_str());
	for(i=0;i<256;i++)
	{
            if(!(i%(16*4)) && i!=0){
				sprintf(buf,"\n");
			...
			if(!((i+1)%4)){
				sprintf(separator,", ");
			} else sprintf(separator,",");
			sprintf(buf,"0x%2.2x%s",(tileProperties[bankViewTable[set+i]/16+i])&0xFF,((i+1)%16)? separator :"\n");
			strcat(str,buf);
...
void __fastcall TFormCHRbit::FormMouseDown(TObject *Sender,
	  TMouseButton Button, TShiftState Shift, int X, int Y)
{
    //not in use, moved to main menu.
	/*
