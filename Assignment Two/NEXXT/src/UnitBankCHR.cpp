...
void __fastcall TFormBankCHR::FormCreate(TObject *Sender)
{
	...
	if(prefStartShowBankSelector) FormBankCHR->Show();
	if(prefStartPinBankSelector)  {
		FormBankCHR->PopupMode=pmExplicit;
		FormMain->PinCHRbankselector1->Checked=true;
	} else{
		FormBankCHR->PopupMode=pmNone;
		FormMain->PinCHRbankselector1->Checked=false;
	}
}
//---------------------------------------------------------------------------
