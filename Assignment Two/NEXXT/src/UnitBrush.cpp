...
void __fastcall TFormBrush::FormCreate(TObject *Sender)
{
	...
	if(prefStartPinBrushes)  {
		FormBrush->PopupMode=pmExplicit;
		FormMain->PinBrushtoolbox1->Checked=true;
	} else{
		FormBrush->PopupMode=pmNone;
		FormMain->PinBrushtoolbox1->Checked=false;
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TFormBrush::oyoradither0degreesrecommended1Click(
      TObject *Sender)
{
	FormMain->SetUndo();
	int tag=0;
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[j][k] = (*ptr_dither_3stepSateen[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::N3stepsateen90degrees1Click(TObject *Sender)
{
		FormMain->SetUndo();
	int tag=0;
	TMenuItem *menuItem = dynamic_cast<TMenuItem*>(Sender);
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[15-k][j] = (*ptr_dither_3stepSateen[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::N3stepsateen180degrees1Click(TObject *Sender)
{
		FormMain->SetUndo();
	int tag=0;
	TMenuItem *menuItem = dynamic_cast<TMenuItem*>(Sender);
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[15-j][15-k] = (*ptr_dither_3stepSateen[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::N3stepsateeb270degrees1Click(TObject *Sender)
{
    	FormMain->SetUndo();
	int tag=0;
	TMenuItem *menuItem = dynamic_cast<TMenuItem*>(Sender);
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[k][15-j] = (*ptr_dither_3stepSateen[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::Ordered4x4Bayermatrix1Click(TObject *Sender)
{
	FormMain->SetUndo();
	int tag=0;
	TMenuItem *menuItem = dynamic_cast<TMenuItem*>(Sender);
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[j][k] = (*ptr_dither_4x4_bayer_low[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::Ordered4x4Bayermatrixmid1Click(TObject *Sender)
{
   FormMain->SetUndo();
	int tag=0;
	TMenuItem *menuItem = dynamic_cast<TMenuItem*>(Sender);
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[j][k] = (*ptr_dither_4x4_bayer_mid[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::Ordered4x4Bayermatrixhighhalf1Click(
      TObject *Sender)
{
    FormMain->SetUndo();
	int tag=0;
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[j][k] = (*ptr_dither_4x4_bayer_hi[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::Ordered4x4Bayermatrixevens1Click(
      TObject *Sender)
{
    FormMain->SetUndo();
	int tag=0;
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[j][k] = (*ptr_dither_4x4_bayer_evens[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::Ordered4x4Bayermatrixodd1Click(TObject *Sender)
{
   FormMain->SetUndo();
	int tag=0;
	TMenuItem *menuItem = dynamic_cast<TMenuItem*>(Sender);
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[j][k] = (*ptr_dither_4x4_bayer_odds[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::N3stepsateen0degreesrighttoleftgrowshimmerfree1Click(
      TObject *Sender)
{
	FormMain->SetUndo();
	int tag=0;
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[j][k] = (*ptr_dither_3stepSateen_left[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::N3stepsateenrightgrowthshimmerfree1Click(
      TObject *Sender)
{
	FormMain->SetUndo();
	int tag=0;
...
void __fastcall TFormBrush::Zigzagleftthenright3stepsateenshimmerfree1Click(
      TObject *Sender)
{
	FormMain->SetUndo();
	int tag=0;
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[j][k] = (*ptr_dither_3stepSateen_zigzag[cnt])[k][j];
			}
		}
		cnt++;
	}
...
void __fastcall TFormBrush::Zigzagleftthenright3stepsateenshimmerfree2Click(
      TObject *Sender)
{
   	FormMain->SetUndo();
	int tag=0;
	...
	for(int i=set;i<end;i++){
		...
		for(int j=0;j<16;j++){
			for(int k=0;k<16;k++){
				(*ptr_tableBrush[i])[j][k] = (*ptr_dither_3stepSateen_zagzig[cnt])[k][j];
			}
		}
		cnt++;
	}
